...
class xlsx2py(object):
	"""
	将excel数据导出为py文件 使用过程需要进行编码转换孙东东
	"""
...
###############export to  py部分######################
	...
	def dic_to_lua_str(self, data, layer):
 
		d_type = type(data)
		if  d_type is bytes or d_type is str:
			return "'" + data + "'"
		elif d_type is bool:
			if data:
				return 'true'
			...
			for k,v in data.items():
				...
				if layer == 0:
					lua_str += "\n"
					lua_str += self.space_str(layer+1)
				if k.isdigit():
					lua_str += '[' + k + ']'
				else:
					if type(v) is dict:
